...
var PlaybackGameRoom = cc.Node.extend({
    ...
    doNext: function () {
        ...
        if (commands.length > 0) {
	        ...
	        if (command.aid === const_val.DISCARD_FORCE) {
                let player = h1global.player();
		        player.curGameRoom.discardStateList[command.serverSitNum] = const_val.DISCARD_FORCE;
		        ...
		        if(player.curGameRoom.playerInfoList[command.serverSitNum]["sex"] === 1) {
			        cc.audioEngine.playEffect('res/sound/' + cc.sys.localStorage.getItem("VOICE_LANGUAGE") + '_voice/' + 'male/sound_man_ting.mp3');
		        } else {
			        cc.audioEngine.playEffect('res/sound/' + cc.sys.localStorage.getItem("VOICE_LANGUAGE") + '_voice/' + 'female/sound_woman_ting.mp3');
		        }
		        return;
	        }
