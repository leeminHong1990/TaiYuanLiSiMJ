...
var GameRoomUI = UIBase.extend({
	...
	show_ting_panel:function(is_show) {
		...
		var player = h1global.player();
		var handTilesNum = player.curGameRoom.handTilesList[player.serverSitNum].length;
		var standTileNum = player.curGameRoom.standTileList[player.serverSitNum].length;
		if (handTilesNum < 4 && handTilesNum === standTileNum) {
			this.rootUINode.getChildByName("ting_operation_panel").getChildByName("guo_btn").setVisible(false);
		}
	},
	...
	_setTileMask:function () {
		...
		for( var i = 0 ; i < handTiles.length ; i++) {
			...
			if (i < temp_standTileList.length && temp_standTileList.indexOf(handTiles[i]) >= 0 && player.canTingTiles.indexOf(handTiles[i]) >= 0) {
	...
	init_player_tile_panel:function(){
		...
		function touch_func(touch_pos, eventType, sender){
        	// cc.log(handTiles)
			let effective_width = self.touch_state === const_val.PLAYER_TOUCH_SELF_STATE ? config.real_width * handTiles.length + offset_x + const_val.CLEARANCE : config.real_width * handTiles.length + const_val.CLEARANCE
			if (touch_pos.x >= -const_val.CLEARANCE && touch_pos.x < effective_width && touch_pos.y >= 0 && touch_pos.y <= hand_panel_height) {
	            ...
	            if (touch_idx < standTileList.length) {
		            touch_pos.x = touch_pos.x + const_val.CLEARANCE;
		            touch_idx = Math.floor(touch_pos.x / config.real_width)
	            }
	            var touch_idx_list = [];
	            for (var i = 0 ; i < player.canTingTiles.length ; i++) {
	            	for (var j = 0 ; j < player.curGameRoom.standTileList[player.serverSitNum].length ; j++) {
			            if (player.curGameRoom.standTileList[player.serverSitNum][j] === player.canTingTiles[i]) {
			            	if (touch_idx_list.indexOf(j) >= 0) {continue;}
		...
		img.setContentSize(cc.size(const_val.CLEARANCE, player_hand_panel.getContentSize().height));
		img.setAnchorPoint(0, 0);
		this.rootUINode.getChildByName("player_tile_panel0").addChild(img);
		img.setPosition(-const_val.CLEARANCE + player_hand_panel.getPositionX(), 0);
                    ...
                    if(isPlayer){
	                    if(discardState === const_val.DISCARD_FORCE){
		                    ...
		                    if (tempStandTileList.indexOf(handTilesList[i]) >= 0) {
			                    this.mark_same_tile(tile_img);
			                    tile_img.setPositionX(tile_img.getPositionX() - const_val.CLEARANCE);
                    ...
                    if(isPlayer){
                        ...
                        if (i < standTileList.length) {
	                        ...
	                        if (!(handTilesList.length % 3 === 2 && i === handTilesList.length - 1)) {
		                        if (idx === 1) {
			                        tile_img.setPositionY(tile_img.getPositionY() + const_val.CLEARANCE);
		                        } else if (idx === 2) {
			                        tile_img.setPositionX(tile_img.getPositionX() - const_val.CLEARANCE);
		                        } else if (idx === 3) {
			                        tile_img.setPositionY(tile_img.getPositionY() - const_val.CLEARANCE);
	...
	update_player_up_tiles: function (serverSitNum) {
		...
		if (idx === 0 && isLeftShift) {
			cur_player_tile_panel.setPositionX(-const_val.CLEARANCE);
		}
    ...
    update_canwin_tile_panel:function(select_tile){
        ...
        var player = h1global.player();
        if (player.curGameRoom.standTileList[player.serverSitNum].indexOf(select_tile) < 0 && select_tile !== const_val.WINTIPS_BTN_DISPLAY) {
	        canwin_tile_panel.setVisible(false);
        	return;
        }
