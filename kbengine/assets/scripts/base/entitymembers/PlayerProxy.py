...
class PlayerProxy(object):
	...
	def __init__(self, avt_mb, owner, idx):
		...
		self.canWinTiles = []
		# 杠牌算分列表
		self.kong_score_list = [0] * self.owner.player_num
	...
	def drawTile(self, tile, is_first = False):
		...
		if not is_first:
			...
			standTileList = [player.standTileList if player is not None and idx == self.idx else [0] * len(player.standTileList) for idx,player in enumerate(self.owner.players_list)]
			self.mb.postOperation(self.idx, const.OP_DRAW, [tile,], 0, standTileList)
		if self.discard_state == const.DISCARD_FORCE:
		...
		if self.discard_state == const.DISCARD_FORCE:
			...
			if self.buckle == 1:
				if tile not in self.standTileList:
					self.mb.doOperationFailed(const.OP_ERROR_STATE)
					return
				self.standTileList.remove(tile)
				self.canWinTiles = self.owner.getCanWinTiles(list(self.tiles), const.OP_DRAW_WIN, self.idx)
