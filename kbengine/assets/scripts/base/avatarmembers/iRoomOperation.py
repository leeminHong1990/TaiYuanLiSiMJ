...
class iRoomOperation(object):
	...
	def createRoom(self, create_dict):
		...
		if game_round not in const.GAME_ROUND \
				...
				or len(play_list) != 2 \
				or play_list[0] not in const.SAME_SUIT_MODE \
				or play_list[1] not in const.SAME_SUIT_LOONG \
		...
		def callback(content):
			...
			try:
				...
				params ={
					...
					'same_suit_mode'	: play_list[0],
					'same_suit_loong'	: play_list[1],
			...
			if room.room_type == const.AGENT_ROOM:
				if room.pay_mode == const.AGENT_PAY_MODE:
					...
					if switch.DEBUG_BASE:
						...
						utility.update_card_diamond(self.accountName, -card_cost, -diamond_cost, callback, "TaiYuanLiSi RoomID:{}".format(room.roomID))  # reason 必须为英文
	...
	def startGame(self, dealer_idx, tile_list, wreaths_list, kingTiles, prevailing_wind, player_wind_list, diceList, swap_list, standTileList):
		if getattr(self, 'client', None):
			self.client.startGame(dealer_idx, tile_list, wreaths_list, kingTiles, prevailing_wind, player_wind_list, diceList, swap_list, standTileList)

	def postOperation(self, idx, aid, tile_list, buckle = 0, standTileList = [[],[],[],[]]):
		if getattr(self, 'client', None):
			self.client.postOperation(idx, aid, tile_list, buckle, standTileList)
