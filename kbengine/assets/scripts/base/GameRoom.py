...
class GameRoom(BaseEntity, GameObject, iRoomRules):
			...
			try:
				...
				if switch.DEBUG_BASE == 0 and self.current_round < 1 and self.agent and self.pay_mode == const.AGENT_PAY_MODE:
					...
					utility.update_card_diamond(self.agent.accountName, card_cost, diamond_cost,
												Functor(callback, self.roomID, self.agent.userId), "TaiYuanLiSi drop AgentRoomID:{}".format(self.roomID))  # reason 必须为英文
			except:
				pass
	...
	def broadcastOperation2(self, idx, aid, tile_list = None):
		...
		for i, p in enumerate(self.players_list):
			if p and i != idx:
				...
				p.mb.postOperation(idx, aid, tile_list, 0, standTileList)
	...
	def pay2StartGame(self):
		""" 开始游戏 """
		DEBUG_MSG("room:{},curround:{},game_round:{},hand_prepare:{} same_suit_mode:{} same_suit_loong:{} pay2StartGame state:{}".format(self.roomID, self.current_round,
		self.game_round, self.hand_prepare, self.same_suit_mode, self.same_suit_loong,
		self.state))
		...
		if self.current_round == 1:
			...
			if self.pay_mode == const.NORMAL_PAY_MODE:
				...
				reason = "TaiYuanLiSi RoomID:{}".format(self.roomID)
				...
				reason = "TaiYuanLiSi Club:{} RoomID:{}".format(self.club.clubId, self.roomID)
				...
				if self.club:
					reason = "TaiYuanLiSi Club:{} AA RoomID:{}".format(self.club.clubId, self.roomID)
				else:
					reason = "TaiYuanLiSi AA RoomID:{}".format(self.roomID)
	...
	def startGame(self, beginTilesList, swap_list):
		...
		for i,p in enumerate(self.players_list):
			if p and p.mb:
				...
				standTileList = [player.standTileList if player is not None and idx == i else [0] * len(player.standTileList) for idx,player in enumerate(self.players_list)]
				p.mb.startGame(self.dealer_idx, beginTilesList[i], self.wreathsList, self.kingTiles, self.prevailing_wind, self.windsList, diceList, swap_list, standTileList)
	...
	def broadcastOperation(self, idx, aid, tile_list = None, buckle = 0):
		...
		for i, p in enumerate(self.players_list):
			if p is not None:
				standTileList = [player.standTileList if player is not None and idx == i else [0] * len(player.standTileList) for idx,player in enumerate(self.players_list)]
				p.mb.postOperation(idx, aid, tile_list, buckle, standTileList)
